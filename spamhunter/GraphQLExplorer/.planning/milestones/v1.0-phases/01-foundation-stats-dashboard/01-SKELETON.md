...
| HTTP method | **POST always** (`preferGetMethod: false` on the urql Client) | The lens serves the GraphiQL **IDE (HTML) on `GET /graphql`** and executes the API only on **`POST /graphql`** (contract § 1). urql's GET-for-queries default lands on GraphiQL → HTML parse failure → spurious NETWORK error. All operations (Phases 2–4 included) MUST POST. |
