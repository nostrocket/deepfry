...
func TestHandleFilterNotice_CaseInsensitive(t *testing.T) {
	...
	if rs.filterCap != 50 {
		t.Fatalf("expected filterCap 50 after case-insensitive match, got %d", rs.filterCap)
	}
}
...
func TestHandleFilterNotice_Floor(t *testing.T) {
	rs := &relayState{url: "wss://example.com", filterCap: 10}
	handleFilterNotice(rs, "filter item too large", 10)
	if rs.filterCap != 10 {
		t.Fatalf("expected filterCap to stay at floor 10, got %d", rs.filterCap)
	}
}
...
func TestHandleFilterNotice_HalveToFloor(t *testing.T) {
	rs := &relayState{url: "wss://example.com", filterCap: 12}
	handleFilterNotice(rs, "filter item too large", 10)
	if rs.filterCap != 10 {
		t.Fatalf("expected filterCap clamped to floor 10, got %d", rs.filterCap)
	}
}
...
func TestHandleFilterNotice_UnrelatedNotice(t *testing.T) {
	...
	if rs.filterCap != 100 {
		t.Fatalf("expected filterCap to remain 100 for unrelated notice, got %d", rs.filterCap)
	}
}
